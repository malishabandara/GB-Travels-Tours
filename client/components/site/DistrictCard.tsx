...
export default function DistrictCard({ d, index }: { d: District; index: number }) {
  return (
    <motion.div
    ...
    >
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        <div className="absolute bottom-3 left-3 right-3 flex items-end justify-between">
          <h3 className="text-white text-lg font-semibold drop-shadow">{d.name}</h3>
        </div>
      </div>
        ...
        <Dialog>
          ...
          <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>Top places we tour in {d.name}</DialogDescription>
            </DialogHeader>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {d.places.map((p) => (
                <figure key={p.name} className="rounded-md overflow-hidden border border-border/60 bg-muted/20">
                  ...
                  <figcaption className="px-3 py-2 text-sm font-medium">{p.name}</figcaption>
                </figure>
              ))}
            </div>
          </DialogContent>
